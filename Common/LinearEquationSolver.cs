...
public static class LinearEquationSolver
{
    ...
    {
        for (var currentRow = 0; currentRow < matrix.RowCount; currentRow++)
        {
            var currentColumn = currentRow; // Use separate variables for clarity

            // Find pivot row
            var pivotRow = currentRow;
            for (var row = currentRow + 1; row < matrix.RowCount; row++)
            {
            ...
            for (var row = currentRow + 1; row < matrix.RowCount; row++)
            {
                ...
                for (var column = currentColumn + 1; column < matrix.ColumnCount; column++)
                {
    ...
    private static T[] BackSubstitute<T>(Matrix<T> matrix) where T : INumber<T>
    {
        ...
        var values = new T[matrix.RowCount];
